...
async function updateBothAutomations() {
    try {
      ...
      if (error) {
        console.error(\`❌ Error actualizando \${auto.name}:\`, error);
      } else {
        console.log(\`✅ \${auto.name} actualizada correctamente\`);
      }
    }
    
    console.log('\n🎉 ¡Ambas automatizaciones ahora tienen el diseño profesional!');
    console.log('📧 Usa cualquiera de las dos para ver el nuevo template HTML elegante');
