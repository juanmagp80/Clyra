-- Script para agregar automatización de Cliente Inactivo
-- Agrega la automatización que detecta clientes sin actividad automáticamente

DO $$
DECLARE
    user_uuid uuid;
BEGIN
    -- Obtener el ID del usuario actual (reemplaza con tu email)
    ...
    IF user_uuid IS NULL THEN
        RAISE NOTICE 'Usuario no encontrado. Actualiza el email en el script.';
        RETURN;
    END IF;

    -- Insertar la automatización de cliente inactivo
    ...
    RAISE NOTICE 'Automatización de Cliente Inactivo agregada/actualizada exitosamente para usuario: %', user_uuid;
END $$;
